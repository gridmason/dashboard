...
- **Realization (D-E4.1 / FR-11).** The verifying SW is a standalone ES-module worker emitted to the bundle root (`federated-sw.js`) so it can claim scope `/`. Its per-fetch enforcement table is the federated boot's merged `url → hash` map (`src/boot/federated-boot` `urlHashes`); the page hands it over by `postMessage` after the SW controls the page and awaits the SW's ack, and the SW persists it (Cache API) so a restarted worker re-hydrates rather than passing bytes through unverified. **Guarded origins are derived from the table itself** (the origins of the claimed URLs): a request to a guarded origin the table does not claim is refused, everything else passes through — that is how per-URL trust is realized without a separate origin list. "Import map assembled only after SW control" is realized by gating the moment a federated remote **becomes mountable** (the federated-host install) on SW control; a federated remote loads by its verified absolute URL through the SW, so the shell does not additionally inject a live `<script type="importmap">` (D-E3 decision, carried forward). **Dev-mode decision (resolves the SW-in-dev ergonomics open question):** a development build **bypasses the SW with loud console labeling** — the Vite dev server serves modules the verifying SW cannot sit in front of without fighting HMR, so `import.meta.env.DEV` gates the bypass and it is dead-stripped from production, which always verifies. The federated path is never weakened: dev bypass is a dev-only convenience, not a production posture.
...
- **v0: reload on gate change.** The assembled import map is injected once per page load; a native import map cannot be mutated after the first module resolves. So in v0 a **gate change takes effect on a full reload** — the shell re-runs gate snapshot → resolution → assembly on boot, not against a live map. Dynamic re-assembly of the running map (adding/removing a remote without a reload) is a documented follow-up, not a v0 guarantee (D-E3.1 / issue #15 risk note). A newly disabled remote is dropped at the gate snapshot before it reaches the map (kill switch above), so failing closed never needs the deferred dynamism.
