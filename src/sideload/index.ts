 ...
 * wraps the router with {@link DevSideloadProvider} and the Add Widget picker
 * renders {@link DevSideloadSection} only in a dev build, so a production build's
 * static `import.meta.env.DEV === false` makes those references dead code and drops
 * this whole subtree from the bundle (proven by `production-gate.test.ts`). The
...
export { DevSideloadSection } from './DevSideloadSection';
